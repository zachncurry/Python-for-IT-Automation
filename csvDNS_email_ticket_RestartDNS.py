...
######################
#REMEDIATION FUNCTIONS Added 7/27/2026
######################
...
def remediation_process(table_device_status):
    remediation_table=[]
    ...
    if any('open' in row for row in remediation_table):
            print("⚠️ Restarting DNS Servers")
            dns_Restart()
            #Holding space for connect & correction function
            #Holding space for re-eval function [after 3 times escalate]
    else:
...
def main():
    ...
    if any('ALERT! Unathorized DNS Server(s)' in row for row in table_device_status):
        ...
        remediation_process(table_device_status)
    else:
